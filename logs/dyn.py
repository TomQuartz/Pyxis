...
# Read data
dir_name = "./convergence_with_slo/"
log_path = {1: dir_name+"dyn-r.log", 2: dir_name+"dyn-t.log"}
res = {j: None for j in range(1, num_subfigs+1)}
partition = {j: [0] for j in range(1, num_subfigs+1)}
...
for j in range(1, num_subfigs+1):
    ...
    for i in res[j]:
        try:
            if i[0] == "rdtsc":
                ...
                partition[j].append(float(i[5].strip()))
    ...
    partition[j] = np.array(partition[j]) / 100
...
for i in range(num_subfigs):
    axes_twinx.append(axes[0][i].twinx())
...
y2_ticks = [25*i for i in range(5)]
y2_label = 'Split Ratio '+r'$\beta$'+' (%)'
...
for j in range(num_subfigs):
    ...
    axes[1][j].set_xlim(left=0, right=30)
    axes[1][j].xaxis.set_ticks_position('bottom')
    axes[1][j].set_xticks(x_ticks)
    axes[0][j].get_xaxis().set_tick_params(direction='in')
    axes[1][j].get_xaxis().set_tick_params(direction='in', pad=6)
    axes_twinx[j].get_xaxis().set_tick_params(direction='in')

# Plot y ticks and label
for j in range(num_subfigs):
    axes[0][j].set_ylim(bottom=y1_bottom[j], top=y1_top[j])
    if j == 0:
        axes[0][j].set_ylabel(y1_label)
        axes[1][j].set_ylabel(y2_label)
    if j == num_subfigs - 1:
        axes_twinx[j].set_ylabel(y3_label, color='red')
    axes[0][j].yaxis.set_ticks_position('left')
    axes[0][j].set_yticks(y1_ticks[j])
    axes[0][j].get_yaxis().set_tick_params(direction='in')
    ...
    axes[1][j].set_ylim(bottom=-3, top=100)
    axes[1][j].yaxis.set_ticks_position('left')
    axes[1][j].set_yticks(y2_ticks)
    axes[1][j].get_yaxis().set_tick_params(direction='in')

# Plot curves
for j in range(1, num_subfigs+1):
    line1,  = axes[0][j-1].plot(time0[j], tput[j], label='Throughput', color='black', linestyle='solid', lw=4.8, zorder=3)
    line2,  = axes[1][j-1].plot(time0[j], partition[j], label='Split Ratio '+r'$\beta$', color='darkorange', linestyle=':', lw=5, zorder=1)
...
for j in range(num_subfigs):
    axes[0][j].spines['top'].set_color('none')
    axes_twinx[j].spines['top'].set_color('none')
    axes[1][j].spines['top'].set_color('none')
    axes[1][j].spines['right'].set_color('none')

# Save the figure
# file_path = 'Dynamic_with_slo.pdf'
file_path = dir_name+'Dynamic_with_slo.png'
